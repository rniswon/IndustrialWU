...
for (year in years){
  for (naics in naics_list) {
    ...
    # Filtering by year and naics
    filtered_boreo_and_pasqualini_df<- boreo_and_pasqualini_coefficients_df %>%
      filter(year == !!year, naics == !!naics)
    
    naics_description <- first(filtered_boreo_and_pasqualini_df$naics_description)
    
    # filtered_boreo_and_pasqualini_df['coefficient_value_per_trillion'] <- filtered_boreo_and_pasqualini_df['coefficient_value'] * 1000000000000
    
    # left join the filtered dataframe to the sf object
    boreo_and_pasqualini_sf <- left_join(l2_ecoregions, filtered_boreo_and_pasqualini_df, by = c("NA_L2CODE" = "l2_ecoregion"))

    # set the geometry column
    boreo_and_pasqualini_sf <- st_set_geometry(boreo_and_pasqualini_sf, "geometry")
    
    boreo_and_pasqualini_map <- ggplot(boreo_and_pasqualini_sf) +
      geom_sf(aes(fill = coefficient_value)) +
      scale_fill_viridis(option = "plasma", na.value = "grey50", trans = "log10") +
      labs(title = paste0("Boreo and Pasqualini coefficients for ", naics, naics_description, ' ', year), fill = "gal / $ of GDP") +
      theme_void()
    
    if (export_figures) 
      {export_figure(boreo_and_pasqualini_map, boreo_and_pasqualini_output_dir, paste0('map_',year, '_', naics), 10, 5)
} 
...
ecoregion_list <- unique(boreo_and_pasqualini_coefficients_df$l2_ecoregion)

# Loop through each ecoregion
for (ecoregion in ecoregion_list) {
  for (naics in naics_list) {
    
    # Filter the DataFrame based on ecoregion and NAICS code
    filtered_boreo_and_pasqualini_df <- boreo_and_pasqualini_coefficients_df %>%
      filter(l2_ecoregion == !!ecoregion, naics == !!naics)
    
    # Get the first NAICS description
    naics_description <- first(filtered_boreo_and_pasqualini_df$naics_description)
    
    # Create the plot
    boreo_and_pasqualini_plot <- ggplot(filtered_boreo_and_pasqualini_df, aes(x = year, y = coefficient_value)) + 
      geom_point() +
      labs(title = paste("Coefficient Value by Year for", naics_description),
           x = "Year",
           y = "Coefficient Value") + 
      theme_minimal()
    ...
    if (export_figures) {
      export_figure(boreo_and_pasqualini_plot, boreo_and_pasqualini_output_dir, 
                    paste0('plot_', ecoregion, '_', naics), 10, 5)
    }
  }
}
