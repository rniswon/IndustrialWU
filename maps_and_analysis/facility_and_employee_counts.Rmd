...
remove_individual_datasets <- function(df){
  df <- df %>%
...
inwu_facility_csv <- paste0(inwu_model_folder,
                            "/INWU_task_folders/Site_selection/Industrial_site_list/USEPA_HIFLD_EIA_PPP_facility_v5.1.csv")
...
if (only_include_compilation) {
  ...
  summary_inwu_df_naics3 <-
    remove_individual_datasets(summary_inwu_df_naics3) 
...
nationwide_estab_summary_plot <- ggplot(summary_estab_long, aes(x = NAICS3, y = sum_estab, shape = estab_type)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_log10() +
  scale_shape_manual(values = c(1, 2, 3, 4, 5))  # Different shapes for each type
...
if (only_include_compilation) {
  ...
  summary_inwu_df_county <-
    remove_individual_datasets(summary_inwu_df_county) 
...
for (naics in naics_list){
  ...
  naics_map_df <- summary_inwu_df_county %>%
    ...
    mutate(sum_emp_CBP = as.integer(sum_emp_CBP),
           sum_estab_CBP = as.integer(sum_estab_CBP)) %>%
    st_as_sf()
  
  scale_max <- max(naics_map_df[, estab_columns], na.rm = TRUE)
  scale_min <- min(naics_map_df[, estab_columns], na.rm = TRUE)
